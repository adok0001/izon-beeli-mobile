...
// ─── Generators ────────────────────────────────────────────────────────────
function buildCourses(languageId, nativeName) {
...
function buildLessons(languageId, def) {
...
export const STUB_COURSES = stubLanguages.flatMap((lang) => buildCourses(lang.id, lang.nativeName));
/** All stub LessonData — spread into ALL_LESSONS in lib/data/lessons/index.ts */
export const STUB_LESSONS = stubLanguages.flatMap((lang) => COURSE_DEFS.flatMap((def) => buildLessons(lang.id, def)));
/** Generate stub courses + lessons for a single language on demand. */
export function stubForLanguage(lang) {
    return {
