...
export function useCreateSession() {
  ...
  return useMutation({
    mutationFn: async (params: {
      ...
      return apiFetch<GameSession>("/multiplayer/sessions", {
        method: "POST",
        token: token!,
...
export function useJoinSession() {
  ...
  return useMutation({
    mutationFn: async (inviteCode: string) => {
      const token = await getToken();
      return apiFetch<GameSession>("/multiplayer/sessions/join", {
        method: "POST",
        token: token!,
...
export function useSessionDetails(sessionId: string | undefined) {
  ...
  return useQuery({
    queryKey: ["multiplayer", "session", sessionId],
    queryFn: async () => {
      const token = await getToken();
      return apiFetch<GameSession>(`/multiplayer/sessions/${sessionId}`, {
        token: token!,
      });
    },
...
export function useJoinMatchmaking() {
  ...
  return useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["multiplayer", "matchmaking"] });
    },
  });
}
...
export function useLeaveMatchmaking() {
  ...
  return useMutation({
    mutationFn: async () => {
      const token = await getToken();
      return apiFetch("/multiplayer/matchmaking/queue", {
        method: "DELETE",
        token: token!,
      });
    },
...
export function useMatchmakingStatus(enabled: boolean) {
  ...
  return useQuery({
    ...
    queryFn: async () => {
      const token = await getToken();
      return apiFetch<{ status: string; session?: GameSession }>(
        "/multiplayer/matchmaking/status",
        { token: token! }
      );
    },
    enabled: !!isSignedIn && enabled,
    refetchInterval: 3000, // Poll every 3s while queued
  });
}
