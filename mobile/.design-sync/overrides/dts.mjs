...
function projectFor(pkgDir, typesRoot) {
  ...
  // FORK: this DS has no shipped .d.ts (Expo app — .tsx source only). Resolve
  // the `@/*` path alias and add the .tsx source tree to the project so
  // propsBodyFor finds each `<Name>Props` interface in source.
  ...
  // FORK: include component/lib/type source so <Name>Props resolves from .tsx.
  for (const g of ['components', 'lib', 'constants', 'hooks', 'store', 'types']) {
    const d = join(pkgDir, g);
    if (existsSync(d)) project.addSourceFilesAtPaths([`${d}/**/*.{ts,tsx}`, `!${d}/**/*.{test,spec,stories}.*`, `!${d}/**/node_modules/**`]);
  }
  console.error(`  [DTS] parsed ${project.getSourceFiles().length} source files from ${pkgDir}`);
