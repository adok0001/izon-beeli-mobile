          ...
          <View className="mt-6 px-5">
            ...
            <View className="gap-2">
              {entries.map((entry) => (
                <View
                  key={entry.id}
                  className="rounded-2xl border border-neutral-200 bg-white p-3 dark:border-neutral-700 dark:bg-neutral-900"
                >
                  <View className="flex-row items-center justify-between">
                    <View className="flex-1 pr-3">
                      <Text className="text-base font-semibold text-neutral-900 dark:text-white">{entry.word}</Text>
                      <Text className="text-sm text-neutral-500 dark:text-neutral-400">{entry.english}</Text>
                    </View>
                    <View className="flex-row gap-2">
                      <Pressable onPress={() => startEdit(entry)} className="rounded-full bg-neutral-100 p-2 dark:bg-neutral-800">
                        <IconSymbol name="gearshape.fill" size={14} color="#6b7280" />
                      </Pressable>
                      <Pressable onPress={() => confirmDelete(entry.id)} className="rounded-full bg-red-100 p-2 dark:bg-red-900/40">
                  ...
                  <View className="mt-2 flex-row flex-wrap gap-1.5">
                    <View className="rounded-full bg-neutral-100 px-2 py-1 dark:bg-neutral-800">
                      <Text className="text-[10px] font-semibold uppercase text-neutral-600 dark:text-neutral-400">{entry.category}</Text>
                    </View>
