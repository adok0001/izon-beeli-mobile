...
contributionsRouter.post("/", async (c) => {
  ...
  try {
    ...
    await db.insert(feedItems).values({
      ...
      type: "contribution",
      title: feedTitle,
      titleTranslations: { en: feedTitle, fr: feedTitleFr },
      description: feedDesc,
      descriptionTranslations: { en: feedDesc, fr: feedDescFr },
...
contributionsRouter.post("/bulk", async (c) => {
  ...
  await db.insert(feedItems).values({
    ...
    titleTranslations: {
      en: `${inserted.length} new ${languageId} entries`,
      fr: `${inserted.length} nouvelles entrées en ${languageId}`,
    },
    description: `Contributed ${inserted.length} words and phrases to the ${languageId} dictionary`,
    descriptionTranslations: {
      en: `Contributed ${inserted.length} words and phrases to the ${languageId} dictionary`,
      fr: `A contribué ${inserted.length} mots et expressions au dictionnaire ${languageId}`,
    },
    userName: user?.name ?? "User",
    userAvatarUrl: user?.avatarUrl,
    contributionId: null,
