...
export function FidelGrid({ learnedIds, onSelect }: FidelGridProps) {
  ...
  return (
    ...
    >
      {/* Header row */}
      <View className="flex-row px-2 pb-2 pt-4">
        <View className="w-12" />
      ...
      {CONSONANT_FAMILIES.map((family) => (
        <View key={family} className="flex-row items-center px-2 py-1">
          <View className="w-12">
          ...
          {[1, 2, 3, 4, 5, 6, 7].map((order) => {
            ...
            return (
              <Pressable
                ...
                className={`mx-0.5 flex-1 items-center rounded-lg py-2 active:opacity-70 ${
                  learned
