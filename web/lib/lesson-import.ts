...
/**
 * Lesson bulk-import contract (client side). One file is one full lesson: a
 * metadata block (key,value rows), a `---` separator, then the transcript grid
 * (one row per line). The educator can pick several files — each parses to one
 * `{ meta, segments }` lesson, and they import together into the course picked in
 * the UI. The server (`/import/lessons`) owns validation + persistence.
 */
 ...
 * themselves contain commas without quoting. The transcript grid (after `---`)
 * is ordinary CSV. A missing `---` yields empty segments, which the server flags.
 */
