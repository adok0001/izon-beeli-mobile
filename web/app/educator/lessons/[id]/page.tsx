...
export default function LessonDetailPage() {
  ...
  const { data: lesson, isLoading } = useQuery<LessonDetail>({
    queryKey: ["educator-lesson", id],
    queryFn: async () => {
      const token = await getToken();
      return apiFetch<LessonDetail>(`/educator/lessons/${id}`, { token: token! });
    },
  });

  // Initialise segments from server data
  useEffect(() => {
    if (lesson) {
      setSegments(
  ...
  const saveSegments = useMutation({
    mutationFn: async () => {
      const token = await getToken();
      return apiFetch(`/educator/lessons/${id}/segments`, {
        method: "PUT",
        token: token!,
        body: JSON.stringify({
  ...
  const uploadAudio = useMutation({
    mutationFn: async (file: File) => {
      ...
      // Read duration from the audio element after it loads
      const duration = audioRef.current?.duration;
      if (duration && isFinite(duration)) fd.append("duration", String(Math.round(duration)));
      return apiFetch<{ audioUrl: string }>(`/educator/lessons/${id}/audio`, {
        method: "POST",
        token: token!,
        body: fd,
      });
      ...
      <div className="mb-6 flex items-start justify-between gap-4">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-neutral-400 dark:text-neutral-500">{lesson.courseTitle}</span>
          </div>
        ...
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-xs text-neutral-400 dark:text-neutral-500 mt-0.5">
              Each segment is one utterance — text in the target language, English translation, and timestamps for audio sync.
          ...
          >
            ...
            ) : saveSegments.isPending ? (
              "Saving…"
            ) : (
              <><Save className="h-4 w-4" /> Save segments</>
            )}
          </button>
        </div>
          ...
          <div className="mt-6 flex items-center justify-end">
            ...
            >
              {saveOk ? (
                ...
                <><Save className="h-4 w-4" /> Save {segments.length} segments</>
              )}
            </button>
          </div>
        )}
