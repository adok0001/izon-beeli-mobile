  ...
  return (
    <div>
      ...
      <div className="rounded-lg border border-neutral-200 dark:border-neutral-700 bg-neutral-50 dark:bg-neutral-800/50 p-3 space-y-2">
        ...
        {previewUrl && !file && (
          <p className="text-[10px] text-neutral-400 dark:text-neutral-500 truncate">{previewUrl}</p>
        )}
        {file && (
          <p className="text-[10px] text-brand-600 dark:text-brand-400 font-medium truncate">↑ {file.name}</p>
        )}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-neutral-200 dark:border-neutral-800 shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-5 space-y-4">
          ...
          <div>
            <label className="text-xs font-medium text-neutral-600 dark:text-neutral-400 mb-1 block">
              {t("admin.dictionary.fieldExample")}
            </label>
          ...
          <div className="grid grid-cols-2 gap-3">
            <FileUploadField
              label={t("admin.dictionary.fieldAudio")}
              accept="audio/*"
              existingUrl={form.audioUrl}
              file={audioFile}
              onFile={setAudioFile}
              onClear={() => { setAudioFile(null); setForm((f) => ({ ...f, audioUrl: null })); }}
              icon={Mic}
              previewType="audio"
            />
            <FileUploadField
              label={t("admin.dictionary.fieldImage")}
              accept="image/*"
              existingUrl={form.imageUrl}
              file={imageFile}
              onFile={setImageFile}
              onClear={() => { setImageFile(null); setForm((f) => ({ ...f, imageUrl: null })); }}
              icon={ImageIcon}
              previewType="image"
            />
          </div>
...
export default function AdminDictionaryPage() {
  ...
  const { data: entries = [], isLoading } = useQuery<DictEntry[]>({
    ...
    queryFn: async () => {
      ...
      return apiFetch<DictEntry[]>(`/dictionary/admin?languageId=${effectiveLanguage}&limit=500`, { token: token ?? undefined });
    },
  ...
  const createEntry = useMutation({
    mutationFn: async ({ data, audioFile, imageFile }: { data: EntryForm; audioFile: File | null; imageFile: File | null }) => {
      ...
      return apiFetch("/dictionary/admin", { method: "POST", body: JSON.stringify(data), token: token ?? undefined });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "dictionary"] });
  ...
  const updateEntry = useMutation({
    mutationFn: async ({ id, data, audioFile, imageFile }: { id: string; data: EntryForm; audioFile: File | null; imageFile: File | null }) => {
      ...
      return apiFetch(`/dictionary/admin/${id}`, { method: "PATCH", body: JSON.stringify(data), token: token ?? undefined });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "dictionary"] });
      setModal(null);
  ...
  const deleteEntry = useMutation({
    mutationFn: async (id: string) => {
      const token = await getToken();
      return apiFetch(`/dictionary/admin/${id}`, { method: "DELETE", token: token ?? undefined });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "dictionary"] });
      void queryClient.invalidateQueries({ queryKey: ["admin", "stats"] });
  ...
  const editContrib = useMutation({
    mutationFn: async ({ id, data }: { id: string; data: Partial<DictEntry> }) => {
      const token = await getToken();
      return apiFetch(`/contributions/${id}`, { method: "PATCH", body: JSON.stringify(data), token: token ?? undefined });
    },
    onSuccess: () => void queryClient.invalidateQueries({ queryKey: ["admin", "dictionary"] }),
  });

  const reviewContrib = useMutation({
    mutationFn: async ({ id, action }: { id: string; action: "approve" | "reject" }) => {
      const token = await getToken();
      return apiFetch(`/contributions/${id}/review`, { method: "PATCH", body: JSON.stringify({ action }), token: token ?? undefined });
    },
    onSuccess: () => {
      void queryClient.invalidateQueries({ queryKey: ["admin", "dictionary"] });
      void queryClient.invalidateQueries({ queryKey: ["admin", "stats"] });
          ...
          <tbody>
              ...
              <tr>
                <td colSpan={7} className="px-4 py-16 text-center">
                  ...
                  <p className="text-sm text-neutral-400 dark:text-neutral-500">
                    {entries.length === 0 ? t("admin.dictionary.empty") : t("admin.dictionary.noResults")}
                  </p>
                </td>
              </tr>
            )}
            {filtered.map((entry) => {
              ...
              return (
                ...
                <tr
                  className={cn(
                    "border-b border-neutral-100 dark:border-neutral-800 last:border-0 hover:bg-neutral-50 dark:hover:bg-neutral-800/40 transition-colors",
                ...
                >
                  ...
                  <td className="px-4 py-3 text-neutral-500 dark:text-neutral-400 text-xs hidden lg:table-cell">
                    {entry.pronunciation ?? "—"}
                  </td>
                  {/* Audio column */}
                    ...
                    {entry._source === "contribution" ? (
                      ...
                      <div className="flex items-center gap-1 justify-end">
                        <button
                          ...
                          className="p-1.5 rounded-lg text-neutral-400 hover:text-brand-600 hover:bg-brand-50 dark:hover:bg-brand-900/20 transition-colors"
                          title={t("admin.dictionary.edit")}
                        >
                          <Edit2 className="h-3.5 w-3.5" />
                        </button>
                        <button
                          onClick={() => {
                            if (globalThis.confirm(t("admin.dictionary.confirmDelete"))) {
                              deleteEntry.mutate(entry.id);
                            }
                          }}
                          disabled={deleteEntry.isPending}
                          className="p-1.5 rounded-lg text-neutral-400 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 transition-colors"
                          title={t("admin.dictionary.delete")}
                        >
                  ...
                  <tr className="bg-teal-50/50 dark:bg-teal-900/10">
                    <td colSpan={7} className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        ...
                        {/* eslint-disable-next-line jsx-a11y/media-has-caption */}
                        <audio controls autoPlay className="flex-1 h-8" src={entry.audioUrl ?? undefined} />
                      </div>
                    </td>
                  </tr>
                )}
